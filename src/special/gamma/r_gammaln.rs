...
pub(crate) fn r_lnabsgamma<T>(x: T) -> T
where
    T: Float + FloatSciConst + SubAssign + MulAssign + DivAssign + AddAssign + RealGammaLnConsts,
{
    ...
    if x.is_sign_negative() {
        ...
        return euler_reflection_prefactor(x).abs().ln() - r_lnabsgamma(-x);
    }
...
mod tests {
    ...
    fn test_gammaln() {
        for i in 1..10 {
            assert_almost_eq!(
                r_lnabsgamma(i as f64),
                ((i - 1).factorial() as f64).ln(),
                PRECISION
            );
            assert_eq!(r_lnabsgamma(-i as f64), f64::INFINITY);
        }

        // Evaluated at Rationals
        assert_almost_eq!(
            r_lnabsgamma(1.0 / 3.0),
            0.985420646927767069187174036977,
            PRECISION
        ); // OEIS: A256165
        assert_almost_eq!(
            r_lnabsgamma(0.25),
            1.28802252469807745737061044021,
            PRECISION
        ); // OEIS: A256166
        assert_almost_eq!(
            r_lnabsgamma(0.20),
            1.52406382243078452488105649392,
            PRECISION
        ); // OEIS: A256167
        assert_almost_eq!(
            r_lnabsgamma(1.0 / 6.0),
            1.71673343507824046052784630958,
            PRECISION
        ); // OEIS: A255888
        assert_almost_eq!(
            r_lnabsgamma(1.0 / 7.0),
            1.87916927159583583645595640934,
            PRECISION
        ); // OEIS: A256609
        assert_almost_eq!(
            r_lnabsgamma(1.0 / 8.0),
            2.01941835755379634532029052116,
            PRECISION
        ); // OEIS: 255306
        assert_almost_eq!(
            r_lnabsgamma(1.0 / 9.0),
            2.14273180037669310488040788489,
            PRECISION
        ); // OEIS: A256610
        assert_almost_eq!(
            r_lnabsgamma(0.1),
            2.25271265173420595986970164636,
            PRECISION
        ); // OEIS: A256612
        assert_almost_eq!(
            r_lnabsgamma(1.0 / 11.0),
            2.35193461079879276046368095764,
            PRECISION
        ); // OEIS: A256611
        assert_almost_eq!(
            r_lnabsgamma(1.0 / 12.0),
        ...
        // Other important
        assert_almost_eq!(
            r_lnabsgamma(PI.recip()),
            1.03364612576558270648553745533,
            PRECISION
        ); // OEIS: A257957

        assert_almost_eq!(r_lnabsgamma(12.5), 18.73434751193644570163, PRECISION);
        assert_almost_eq!(r_lnabsgamma(13.34), 20.8506330413774776, PRECISION);
        assert_almost_eq!(r_lnabsgamma(-34.5), -89.2102003799689880, 1e-13); // Taken from Wolframalpha
        assert_almost_eq!(r_lnabsgamma(-40.2), -109.3852746800507908, 1e-13); // Taken from Wolframalpha
        assert_almost_eq!(r_lnabsgamma(13.5), 21.2600761562447011, PRECISION); // Taken from Wolframalpha
        assert_almost_eq!(r_lnabsgamma(14.5), 23.8627658416890849, PRECISION); // Taken from Wolframalpha
        assert_almost_eq!(
            r_lnabsgamma(150.0 + 1.0e-12),
            600.0094705553324354,
            PRECISION
        );
        ...
        assert_almost_eq!(
            r_lnabsgamma(-1.72),
            0.95458292505988099545337076566,
            PRECISION
        );
    }
}
